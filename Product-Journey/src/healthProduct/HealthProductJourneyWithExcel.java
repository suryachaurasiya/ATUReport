...
public class HealthProductJourneyWithExcel extends ExtentBaseClass {
	...
	@Test(description = "Insert mobile no in mobile no input box", priority = 8)
	public void mobile() throws Exception {

		try {
			test = extent.createTest("Insert mobile no in mobile no input box", "Input mobile number");
			WebElement mobile = driver.findElement(uimap.getLocator("mobile_no_input"));
			mobile.click();
			Thread.sleep(1000);
			mobile.sendKeys(datafile.getData("mobilenoinnput"));
			if (datafile.getData("mobilenoinnput").equals("9873008200")) {
				test.pass(datafile.getData("mobilenoinnput") + " equal " + "9873008200");
			} else
				test.log(Status.FAIL, datafile.getData("mobilenoinnput") + " doesn't equal " + "9873008200");
	...
	public void QuotesBuyNow() throws Exception {
		...
		for (int i = 1; i <= buyNowElement.size() - 81; i+=2) {
			Thread.sleep(10000);
